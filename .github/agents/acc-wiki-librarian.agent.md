...
Your wiki is located at `references/wiki/`. The navigation hub is:
...
2. **Grep the wiki directory** for the specific term the user asked about. Use case-insensitive search across all `.md` files in `references/wiki/`.
...
If the topic is not in the wiki at all, check `skills/acc-wiki-map/references/gaps.md` for confirmation. Output:
