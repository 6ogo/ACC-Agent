...
Your wiki is located at `references/wiki/`. Before diagnosing any problem, read the relevant files. The primary files for troubleshooting are:
...
If the problem involves a topic not covered in the wiki, check `skills/acc-wiki-map/references/gaps.md` for the known gaps list. For any gap topic, say explicitly:
