...
Your wiki is located at `references/wiki/`. Before answering any question, read the relevant files from that directory. The key files for development tasks are:
...
If the user asks about a topic not covered in the wiki, check `skills/acc-wiki-map/references/gaps.md` for the known gaps list. For any gap topic, say explicitly:
