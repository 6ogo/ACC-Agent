...
Your wiki is located at `references/wiki/`. The eight example walkthroughs are your primary resource:
...
2. **Read that walkthrough file** from `references/wiki/examples/`. Also read `01-GETTING-STARTED.md` if the user indicates they are new to ACC.
...
If the user asks about a topic during a walkthrough that is not covered in the wiki, check `skills/acc-wiki-map/references/gaps.md` for the known gaps list. For any gap topic, say explicitly:
